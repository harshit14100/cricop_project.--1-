...
import { ChevronRight, ChevronLeft, Trophy, Users, MapPin, UserCheck, Coins, Share2, CheckCircle, Copy, MessageCircle } from 'lucide-react'
...
export default function StartMatchPage() {
  ...
  const copyLink = () => {
    navigator.clipboard.writeText(`https://cricop.com/match/${createMatch.data?.id}`)
    setCopied(true)
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <motion.div
        ...
        className="mb-8"
      >
        <h1 className="text-2xl md:text-3xl font-bold text-white">Start New Match</h1>
        <p className="text-sm text-white/50 mt-1">Set up your match in a few easy steps</p>
      </motion.div>
            ...
            return (
              <div key={step.id} className="flex items-center flex-1">
                <div className="flex flex-col items-center">
                  <motion.div
                    animate={{
                      backgroundColor: isActive ? '#3b5bdb' : isCompleted ? '#0a1628' : 'rgba(255,255,255,0.05)',
                      borderColor: isActive ? '#5c7cfa' : isCompleted ? '#3b5bdb' : 'rgba(255,255,255,0.1)',
                    }}
                  ...
                  >
                    {isCompleted ? (
                      ...
                      <step.icon className={cn('h-5 w-5', isActive ? 'text-white' : 'text-white/40')} />
                    )}
                  </motion.div>
                  <span className={cn('text-xs mt-2 hidden md:block', isActive ? 'text-white' : 'text-white/40')}>
                    {step.title}
                  </span>
                </div>
                {i < steps.length - 1 && (
                  <div className={cn('h-0.5 flex-1 mx-2 rounded-full', isCompleted ? 'bg-blue-500' : 'bg-white/10')} />
                )}
              </div>
            )
          })}
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <Card className="glass-card p-6 md:p-8">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => setMatchData({ ...matchData, matchType: v as any })}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                ...
                {matchData.matchType === 'custom' && (
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setMatchData({ ...matchData, totalOvers: parseInt(e.target.value) || 20 })}
                      min={1}
                      max={50}
                    />
                  </div>
            ...
            {currentStep === 2 && (
              <div className="space-y-6">
                <h2 className="text-xl font-semibold text-white">Select Teams</h2>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(v) => setMatchData({ ...matchData, teamAId: v })}
                    >
                      <SelectTrigger>
                      ...
                      <SelectContent>
                        ...
                        {teamsData?.teams.map(t => (
                          ...
                          <>
                            <SelectItem value="t1">Mumbai Indians</SelectItem>
                            <SelectItem value="t2">Chennai Super Kings</SelectItem>
                            <SelectItem value="t3">Royal Challengers</SelectItem>
                          </>
                        )}
                      </SelectContent>
                    </Select>
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(v) => setMatchData({ ...matchData, teamBId: v })}
                    >
                      <SelectTrigger>
                        <SelectValue placeholder="Select team" />
                      </SelectTrigger>
                      <SelectContent>
                        ...
                        {teamsData?.teams.map(t => (
                          ...
                          <>
                            <SelectItem value="t2">Chennai Super Kings</SelectItem>
                            <SelectItem value="t4">Kolkata Knight Riders</SelectItem>
            ...
            {currentStep === 3 && (
              <div className="space-y-6">
                <h2 className="text-xl font-semibold text-white">Playing XI</h2>
                <p className="text-sm text-white/50">Select 11 players for each team</p>

                <div className="space-y-4">
                  <div className="p-4 rounded-xl bg-white/5 border border-white/10">
                    <h3 className="text-sm font-medium text-white mb-3">Team A Players</h3>
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                      {playersData?.players.slice(0, 11).map(p => (
                        <Badge key={p.id} variant="secondary" className="justify-center py-2">
                          {p.name}
                        </Badge>
            ...
            {currentStep === 4 && (
              <div className="space-y-6">
                <h2 className="text-xl font-semibold text-white">Match Officials</h2>
            ...
            {currentStep === 5 && (
              <div className="space-y-6">
                <h2 className="text-xl font-semibold text-white text-center">Toss Time!</h2>
                ...
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Select
                      ...
                      onValueChange={(v) => setMatchData({ ...matchData, tossWinner: v })}
                    >
                      <SelectTrigger>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <Button
                        type="button"
                        variant={matchData.tossChoice === 'bat' ? 'default' : 'outline'}
                      ...
                      <Button
                        type="button"
                        variant={matchData.tossChoice === 'bowl' ? 'default' : 'outline'}
                        className="h-16 text-lg"
            ...
            {currentStep === 6 && (
              <div className="space-y-6 text-center">
                ...
                <h2 className="text-xl font-semibold text-white">Match Ready!</h2>
                <p className="text-sm text-white/50">Share this link with your team and audience</p>
                ...
                <div className="flex gap-3 justify-center">
                  ...
                  <Button onClick={() => navigate(`/live-scoring/${createMatch.data?.id || 'abc123'}`)}>
                    Start Scoring
                  </Button>
                </div>
              </div>
